...
| 4c | Любой | NFT молчит (нет ни `Excesses`, ни bounce) дольше часа после покупки: сообщение `Settle` завершает сделку как успешную | `Sold` |
...
stateDiagram-v2
    ...
    Transferring --> Sold: Settle (NFT молчит дольше часа)
...
|---|---|---|---|---|
| `OwnershipAssigned` | `0x05138d91` (TEP-62) | только адрес NFT | `WaitingNft` | Депозит принят, если предыдущий владелец = продавец. Ставит `ForSale` |
...
| `Withdraw` | `0x0b5a0003` | только продавец | `WaitingNft`, `Sold`, `Cancelled` | Отправляет продавцу остаток TON, оставляя 0.02 TON на хранение |
| `Settle` | `0x0b5a0004` | любой, но не раньше чем через 3600 с после `Buy` | `Transferring` | Завершает сделку как успешную, если NFT так и не ответил |
...
| Код | Название | Когда |
|---|---|---|
| 101 | `ERR_NOT_EXPECTED_NFT` | сообщение `OwnershipAssigned`, `Excesses` или bounce пришло не от адреса нашего NFT |
| 102 | `ERR_NOT_SELLER` | депозит сделал не продавец, либо `Cancel`/`Withdraw` шлёт не продавец |
...
|---|---|---|---|
| **Поддельное уведомление**: злоумышленник шлёт сообщение с кодом `OwnershipAssigned` и делает вид, что NFT внесён | Уведомление принимается только от адреса `nftAddress`, `sender` берётся из самого сообщения сети, а не из его тела | `handleOwnershipAssigned` | `rejects a fake notification from a random wallet` |
| **Чужой NFT**: пришёл другой NFT либо тот же NFT, но не от продавца | Отправитель должен быть `nftAddress` (ошибка 101), а `prevOwner` должен быть продавцом (ошибка 102) | `handleOwnershipAssigned` | `attack: a different real NFT is rejected by GiftSwap`, `rejects the right NFT if it was sent by someone other than the seller` |
...
| **NFT не отвечает вовсе**: деньги и сделка зависли бы навсегда | `Settle` через час после покупки завершает сделку (тишина после отправки с bounce означает успех); опоздавшие ответы после этого отвергаются или игнорируются | `handleSettle` | `settle works exactly at the deadline…`, `a late excesses or bounce after settle…` |
...
| **Повторная отправка NFT после депозита** | Уведомление принимается только в `WaitingNft` | `handleOwnershipAssigned` | `rejects a second deposit notification` |
...
- *Что делает:* хранит состояние сделки (`WaitingNft` → … → `Sold`/`Cancelled`), принимает сообщения `OwnershipAssigned`, `Excesses`, `Buy`, `Cancel`, `Withdraw`, обрабатывает bounce, отдаёт данные через `swapInfo()`.
...
**`tests/GiftSwap.spec.ts`**: 37 тестов логики самого контракта.
...
**`tests/GiftSwapWithNft.spec.ts`**: 35 интеграционных тестов с тестовым NFT.
- *Что делает:* проверяет весь путь с настоящими цепочками сообщений: депозит, покупка, подтверждение, провал перевода и bounce, `Cancel`, `Withdraw`. В разделе про газ закрепляет замеры: бюджет газа обработчиков, что контракт не истощается, запас `NFT_TRANSFER_MIN`, минимальный `forward_amount`, плата за хранение.
...
| Срок до `Settle` | это выбор, а не замер: при провале bounce приходит за секунды | `SETTLE_TIMEOUT = 3600 с` |
...
npx blueprint test                # 72 теста в Sandbox
...
Отправьте сообщение `Buy` (`0x0b5a0001`, только код операции) с суммой не меньше `цена + 0.06 TON`. Излишек вернётся. Сообщение можно собрать обёрткой (`GiftSwap.sendBuy`, `buyBody()`) в скрипте Blueprint или отправить из кошелька, который умеет отправлять произвольное тело сообщения.
...
- **Повторный `Withdraw` при пустом остатке бесполезен.** Контракт его принимает: продавец нажал его ещё дважды и получил обратно только приложенные 0.03 TON за вычетом комиссии. Вреда нет, только комиссия. Перед выводом стоит смотреть баланс контракта (см. ограничение 8).
- **`Cancel` в `WaitingNft`, когда NFT ещё не внесён в контракт.** NFT отклонил перевод (контракт ему не владелец), приложенные TON остались на контракте как остаток. Они не потеряны: их можно вернуть `Withdraw` (см. ограничение 9).
...
Есть важное следствие для безопасности: хук может вызвать кто угодно, поэтому получатель должен убедиться, что вызывает именно ожидаемый токен-контракт. У нас это `assert (sender == storage.nftAddress)`. Тест `rejects a fake notification from a random wallet` проверяет именно это.
...
1. **`Settle` опирается на предположение «тишина значит успех».** Перевод NFT отправляется с bounce, поэтому при провале bounce приходит за секунды. Если сообщение когда-нибудь задержится больше часа (крайне маловероятно), `Settle` заплатит продавцу до того, как NFT перешёл; если перевод потом провалится, покупатель потеряет цену. Срок (`SETTLE_TIMEOUT`) можно увеличить, компромисс между защитой покупателя и скоростью разблокировки.
...
3. **Доверие к адресу NFT.** Адрес NFT задаёт продавец. Контракт не может проверить ончейн, что NFT действительно перешёл: он полагается на `Excesses` от этого адреса (или на молчание, если сработал `Settle`). Покупатель должен сам сверить `nftAddress` из `swapInfo()` с тем NFT, который хочет купить.
4. **Чужие NFT не возвращаются автоматически.** Если на контракт пришёл NFT от неожиданного адреса, уведомление отвергается, а сам NFT остаётся на контракте: отправителю такого уведомления доверять нельзя.
5. **Нет роялти и комиссии маркетплейса, нет смены цены, один NFT на один контракт.**
...
7. **Аудита не было.**
8. **`Withdraw` не смотрит, есть ли что выводить.** Контракт принимает его и при пустом остатке (остаётся только резерв) и тратит комиссию. Замечено на testnet. Перед выводом проверяйте баланс контракта.
9. **`Cancel` в `WaitingNft`, если NFT не в контракте.** NFT отклонит перевод, а приложенные TON останутся на контракте как остаток (вернуть можно `Withdraw`). Перед `Cancel` убедитесь, что NFT действительно лежит на адресе контракта.
10. **На настоящем NFT не проверены `Cancel` из `ForSale` и `Settle`**, а из кошельков пробовался только Keeper (остальное проверено в Sandbox).
...
- При желании научить контракт не тратить комиссию на пустой `Withdraw` (сейчас это ограничение 8).
...
- Тесты контракта: 72 (37 на логику, 35 с тестовым NFT). Плюс проверка на настоящем testnet (см. раздел выше).
