...
**`tests/GiftSwap.spec.ts`**: 38 тестов логики самого контракта.
...
**`tests/GiftSwapWithNft.spec.ts`**: 38 интеграционных тестов с тестовым NFT.
...
npx blueprint test                # 76 тестов в Sandbox
...
> Проверялась предыдущая версия контракта (её хеш кода указан ниже). После проверки изменены только `SETTLE_TIMEOUT` (1 час → 24 часа) и `Withdraw` (отказ 107 при пустом остатке). Депозит, покупка и выплаты не менялись.
...
- Тесты контракта: 76 (38 на логику, 38 с тестовым NFT). Плюс проверка на настоящем testnet (см. раздел выше).
